...
export class ServerPlayer extends BasePlayer {
    static readonly mugshotInitialDelayMs = 2 * 60 * 1000;
    static readonly mugshotHourlyDelayMs = 60 * 60 * 1000;
    static readonly mugshotJitterMs = 10 * 60 * 1000;
    ...
    #mugshotRefreshTimeout?: ReturnType<typeof setTimeout>;
    ...
    constructor(
        ...
        if (this.license) {
            ...
            this.#scheduleMugshotRefresh(ServerPlayer.mugshotInitialDelayMs);
        } else {
    ...
    /**
     * Updates the cached mugshot in the players database.
     */
    setMugshot(mugshot: string) {
        if (!this.license || !this.isRegistered || !this.isConnected) return false;
        if (!/^data:image\/(?:png|jpeg|webp);base64,[A-Za-z0-9+/=]+$/.test(mugshot)) return false;
        if (mugshot.length > 350_000) return false;
        this.mutateDbData({
            mugshot,
            tsMugshot: now(),
        });
    ...
    #scheduleMugshotRefresh(delayMs: number) {
        const jitter = Math.floor(Math.random() * ServerPlayer.mugshotJitterMs);
        this.#mugshotRefreshTimeout = setTimeout(() => {
            if (!this.isConnected) return;
            this.#fxPlayerlist.requestMugshot(this.netid);
            this.#scheduleMugshotRefresh(ServerPlayer.mugshotHourlyDelayMs);
        }, delayMs + jitter);
    }
    ...
    disconnect() {
        ...
        clearTimeout(this.#mugshotRefreshTimeout);
    }
}
