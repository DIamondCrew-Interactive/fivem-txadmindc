...
import { PlayerDroppedEventType, PlayerJoiningEventType, PlayerUpdatedEventType } from '@shared/socketioTypes';
...
export default class FxPlayerlist {
        ...
        return this.#playerlist
            ...
            .map((p) => {
                return cloneDeep({
                    ...
                    pureName: p!.pureName,
                    license: p!.license,
                    mugshot: p!.dbData ? p!.dbData.mugshot : undefined,
                });
            });
    }
    ...
    requestMugshot(playerId: number) {
        txCore.fxRunner.sendEvent('requestPlayerMugshot', { id: playerId });
    }
        ...
        if (payload.event === 'playerJoining') {
            try {
                ...
                txCore.webServer.webSocket.buffer<PlayerJoiningEventType>('playerlist', {
                    ...
                    mugshot: svPlayer.dbData ? svPlayer.dbData.mugshot : undefined,
                });
            } catch (error) {
                console.verbose.warn(`playerJoining event error: ${(error as Error).message}`);
            }

        } else if (payload.event === 'playerMugshot') {
            try {
                if (typeof payload.id !== 'number') throw new Error(`invalid player id`);
                if (typeof payload.mugshot !== 'string') throw new Error(`invalid mugshot`);
                if (!(this.#playerlist[payload.id] instanceof ServerPlayer)) throw new Error(`player id not found`);
                const svPlayer = this.#playerlist[payload.id]!;
                if (!svPlayer.setMugshot(payload.mugshot)) throw new Error(`mugshot rejected`);
                txCore.webServer.webSocket.buffer<PlayerUpdatedEventType>('playerlist', {
                    mutex,
                    type: 'playerUpdated',
                    netid: svPlayer.netid,
                    mugshot: payload.mugshot,
                });
            } catch (error) {
                console.verbose.warn(`playerMugshot event error: ${(error as Error).message}`);
            }
