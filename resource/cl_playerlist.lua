...
RegisterNetEvent('txcl:plist:reqMugshot', function(requestId)
    ...
    if type(txd) == 'string' and txd ~= '' then
        sendMenuMessage('captureMugshot', {
            requestId = requestId,
            txd = txd,
        })
    end
...
RegisterSecureNuiCallback('mugshotCaptured', function(data, cb)
    if type(data) == 'table' and data.requestId and type(data.image) == 'string' then
        TriggerServerEvent('txsv:plist:mugshotCaptured', data.requestId, data.image)
    end
    cb({})
end)
