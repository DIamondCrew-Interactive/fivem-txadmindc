...
RegisterNetEvent('txsv:plist:mugshotCaptured', function(playerID, mugshot)
    if source <= 0 or source ~= tonumber(playerID) then
        logError('Rejected mugshot event with source ' .. json.encode(source))
        return
    end
    if type(mugshot) ~= 'string' or #mugshot > 350000 or not mugshot:match('^data:image/%a+;base64,') then
